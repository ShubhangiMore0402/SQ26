...
**Author:** Shubhangi More · 
**Repositories:** Zenodo (Repo #1) · Harvard Dataverse (Repo #10)

---

A Python pipeline that finds, downloads, and catalogues publicly available qualitative research projects focused on QDA (Qualitative Data Analysis) files from two public repositories with very different APIs. Metadata is stored in a SQLite database. Files are saved as-is with no transformations at download time; data quality issues are detected and flagged separately rather than silently patched.

**Results:** catalogued **<!-- TODO: N -->** research projects and **<!-- TODO: M -->** files across the two repositories, with a per-file download status recorded for every attempt. *(Replace the placeholders with the real counts from your `files` table, e.g. `SELECT COUNT(*) FROM projects;` and `SELECT COUNT(*) FROM files;`.)*

The interesting engineering problem here was reliability against two inconsistent, partially-documented APIs: handling missing fields, login-gated files, and datasets cross-listed from third-party repositories without dropping records or crashing mid-run.
...
├── downloads/                  # Downloaded datasets (not committed but shared via link)
│   ├── zenodo/
│   │   └── <record_id>/
│   │       └── *.qdpx, *.docx, ...
│   └── harvard-dataverse/
...
The database file is `23137504-seeding.db` and is committed to the root of the repository.
...
result with an open license, all files in the record are downloaded and saved under `downloads/zenodo/<record_id>/`.
